...
$(document).ready(function(){
	...
	$('#passwordInput').popover({
		...
		trigger: 'hover',
		html: true,
		content: '<ul><li>Evite usar seu nome, apelido ou data de nascimento.</li><li>Combine tras maiúsculas e minúsculas, símbolos e números.</li><li>Não use informações pessoais.</li></ul>',
		...
		if(cep != "")
		{
			...
			if(validacep.test(cep))
			{
				...
				$.getJSON("//viacep.com.br/ws/" + cep + "/json/?callback=?", function(dados){
					...
					{
						$('#wait').text('CEP não encontrado!').show();
						$('#logradouroInput').val('');
						$('#bairroInput').val('');
						$('#cidadeInput').val('');
						$('#ufInput').val('');
